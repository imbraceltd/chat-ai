...
from open_webui.utils import audio_service as audio_service
...
@router.post("/stt")
async def stt(request: Request, userContext=Depends(auth_imbrace)):
    try:
        body = await request.json()
        audioUri = body.get("audioUri")
        languageCode = body.get("languageCode")
        vendor = body.get("vendor")
        file = body.get("file")
        print("Transcribing audio file in controller...")
        transcription = await audio_service.transcribe(file, audioUri, languageCode, vendor)
...
@router.post("/tts")
async def tts(request: Request, userContext=Depends(auth_imbrace)):
    try:
        body = await request.json()
        text = body.get("text")
        languageCode = body.get("languageCode")
        ...
        audio_data  = await audio_service.speechify(text, languageCode, vendor)
  
        if not audio_data:
            raise HTTPException(
                status_code=400, 
                detail="Failed to generate audio, the message may be empty."
            )
