...
def get_rf(
    ...
    if reranking_model:
        if engine == "ollama":
            try:
                from open_webui.retrieval.models.rerankers import OllamaReranker
                ...
                log.error(f"OllamaReranker: {e}")
                raise Exception(ERROR_MESSAGES.DEFAULT(e))
        elif engine == "openai":
            try:
                from open_webui.retrieval.models.rerankers import OpenAIReranker
                ...
                log.error(f"OpenAIReranker: {e}")
                raise Exception(ERROR_MESSAGES.DEFAULT(e))
        elif engine == "bedrock":
            try:
                from open_webui.retrieval.models.rerankers import BedrockReranker

                # Credentials come from BEDROCK_* env (same as embedding path).
                rf = BedrockReranker(model=reranking_model)
            except Exception as e:
                log.error(f"BedrockReranker: {e}")
                raise Exception(ERROR_MESSAGES.DEFAULT(e))
        elif any(model in reranking_model for model in ["jinaai/jina-colbert-v2"]):
            try:
    ...
    try:
        ...
        try:
            request.app.state.rf = get_rf(
                ...
                ollama_base_url=request.app.state.config.RAG_OLLAMA_BASE_URL,
...
def save_docs_to_vector_db(
    ...
    try:
        ...
        log.info(
            f"Saving {len(items)} documents to collection {collection_name}"
        )
