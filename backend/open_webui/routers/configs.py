...
async def verify_tool_servers_config(
    request: Request, form_data: ToolServerConnection
):
    """
    Verify the connection to the tool server.
    """
    log.info(
        "[tool_servers.verify] payload=%s",
        form_data.model_dump(),
    )
    try:
        if form_data.type == "mcp":
            log.info(
                "[tool_servers.verify] handling MCP url=%s auth_type=%s",
                form_data.url,
                form_data.auth_type,
            )
            if form_data.auth_type == "oauth_2.1":
                # OAuth 2.1 discovery flow disabled
                pass
                ...
                try:
                    try:
                        ...
                        log.exception(
                            "[tool_servers.verify] MCP client import failed"
                        )
                        raise HTTPException(
                            status_code=400,
                    ...
                    log.info(
                        "[tool_servers.verify] connecting mcp url=%s token=%s",
                        form_data.url,
                        "provided" if token else "absent",
                    )
                    await client.connect(form_data.url, headers=headers)
                    specs = await client.list_tool_specs()
                    log.info(
                        "[tool_servers.verify] fetched %s specs %s",
                        len(specs) if specs else 0,
                        [spec.get("name") for spec in specs[:10]] if specs else [],
                    )
                    return {
                        "status": True,
                        "specs": specs,
                    ...
                    log.exception(
                        "[tool_servers.verify] MCP verification failed url=%s",
                        form_data.url,
                    )
                    raise HTTPException(
                        status_code=400,
            ...
            log.info(
                "[tool_servers.verify] handling OpenAPI url=%s path=%s",
                form_data.url,
                form_data.path,
            )
            token = None
            ...
            log.info(
                "[tool_servers.verify] resolved url=%s token=%s",
                url,
                "provided" if token else "absent",
            )
        ...
        log.exception(
            "[tool_servers.verify] unexpected error url=%s type=%s",
            form_data.url,
            form_data.type,
        )
        raise HTTPException(
