...
def add_file_to_knowledge_by_id(
    ...
    # Check file type to handle CSV and Excel files specially
    file_extension = file.filename.lower().split('.')[-1] if '.' in file.filename else ''
    is_special_file = file_extension in ['csv', 'xlsx', 'xls']
    
    # Only check data for non-special files
    if not file.data and not is_special_file:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        if not is_special_file:
            ...
            # Log that we're skipping vector processing for this file type
            log.info(f"Skipping vector processing for {file.filename} (type: {file_extension})")
    except Exception as e:
        log.debug(f"Error processing file: {str(e)}")
        # Don't fail for special file types that might not support vector embedding
        if not is_special_file:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=str(e),
