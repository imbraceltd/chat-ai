...
# Ensure certain optional API keys exist in the module globals early
if "GOOGLE_VISION_API_KEY" not in globals():
    try:
...
frontend_splash = FRONTEND_BUILD_DIR / "static" / "imbrace.png"

if frontend_splash.exists():
    try:
        shutil.copyfile(frontend_splash, STATIC_DIR / "imbrace.png")
    except Exception as e:
        logging.error(f"An error occurred: {e}")
    ...
    try:
        ...
        if r.ok:
            ...
            if "splash" in data:
                ...
                r = requests.get(url, stream=True)
                if r.status_code == 200:
                    with open(f"{STATIC_DIR}/imbrace.png", "wb") as f:
                        r.raw.decode_content = True
                        shutil.copyfileobj(r.raw, f)
...
# OLM OCR Model Configuration
OLM_API_KEY = os.environ.get("OLM_API_KEY", "")
OLM_HOST = os.environ.get("OLM_HOST", "http://localhost:8001")
OLM_MODEL = os.environ.get("OLM_MODEL", "llama3.2-vision")

# OLM Configuration Object
OLM_CONFIG = {
    "host": OLM_HOST,
    "model": OLM_MODEL,
}
...
# Wrap in PersistentConfig but use non-existent path to always use env value
OPENAI_API_KEYS = PersistentConfig(
    "OPENAI_API_KEYS", "openai.api_keys_env_only", OPENAI_API_KEYS
)
...
OPENAI_API_BASE_URLS = PersistentConfig(
    "OPENAI_API_BASE_URLS", "openai.api_base_urls_env_only", OPENAI_API_BASE_URLS
)
...
# Always use latest value from env, not from PersistentConfig
OPENAI_API_KEY = ""
try:
...
USER_PERMISSIONS_CHAT_MULTIPLE_MODELS = (
    os.environ.get("USER_PERMISSIONS_CHAT_MULTIPLE_MODELS",
                   "True").lower() == "true"
)
...
USER_PERMISSIONS_FEATURES_DIRECT_TOOL_SERVERS = (
    os.environ.get(
        "USER_PERMISSIONS_FEATURES_DIRECT_TOOL_SERVERS", "False").lower()
    == "true"
)
...
ENABLE_AUTOCOMPLETE_GENERATION = PersistentConfig(
    ...
    os.environ.get("ENABLE_AUTOCOMPLETE_GENERATION",
                   "False").lower() == "true",
)
...
ENABLE_PDF_OCR = PersistentConfig(
    "ENABLE_PDF_OCR",
    "rag.enable_pdf_ocr",
    os.environ.get("ENABLE_PDF_OCR", "True").lower() == "true",
)

# Real-OCR engine used to ground Document AI extraction confidence. One of:
# "rapidocr" (default, no extra install), "paddleocr", "tesseract", "azure".
# Only the selected engine is ever imported; only its dependencies need be installed.
DOCUMENT_AI_OCR_ENGINE = PersistentConfig(
    "DOCUMENT_AI_OCR_ENGINE",
    "rag.document_ai_ocr_engine",
    os.getenv("DOCUMENT_AI_OCR_ENGINE", "rapidocr"),
)

# Geometry-preserving image preprocessing (denoise/contrast/upscale) before OCR, to
# improve recognition accuracy — and thus confidence reliability — on the rendered pages.
DOCUMENT_AI_OCR_PREPROCESS = PersistentConfig(
    "DOCUMENT_AI_OCR_PREPROCESS",
    "rag.document_ai_ocr_preprocess",
    os.getenv("DOCUMENT_AI_OCR_PREPROCESS", "True").lower() == "true",
)

# Use strict json_schema (guided decoding) for the vLLM provider's Document AI Step-2
# schema mapping instead of loose json_object. Disable for a vLLM build without guided decoding.
DOCUMENT_AI_VLLM_STRUCTURED_OUTPUT = PersistentConfig(
    "DOCUMENT_AI_VLLM_STRUCTURED_OUTPUT",
    "rag.document_ai_vllm_structured_output",
    os.getenv("DOCUMENT_AI_VLLM_STRUCTURED_OUTPUT", "True").lower() == "true",
)

# When set, the "rapidocr" OCR engine calls an external RapidOCR API server
# (rapidocr_api, POST /ocr) instead of loading the local ONNX model. Accepts a base
# URL (e.g. http://localhost:9003) or the full /ocr endpoint. Empty = use local model.
RAPIDOCR_API_URL = PersistentConfig(
    "RAPIDOCR_API_URL",
    "rag.rapidocr_api_url",
    os.getenv("RAPIDOCR_API_URL", ""),
)
...
RAG_EMBEDDING_CONTENT_PREFIX = os.environ.get(
    "RAG_EMBEDDING_CONTENT_PREFIX", None)
...
RAG_RERANKING_MODEL = PersistentConfig(
    "RAG_RERANKING_MODEL",
    "rag.reranking_model",
    os.environ.get("RAG_RERANKING_MODEL", "cross-encoder/ms-marco-MiniLM-L-12-v2"),
)
...
MONGODB_CONFIG: Dict[str, Any] = {
    # Basic connection
    "openai_host": os.getenv("MONGODB_OPENAI_HOST", "mongodb://localhost:27017"),
    "openai_db_name": os.getenv("MONGODB_OPENAI_DB_NAME", "openai_db"),
    # Vector search configuration
    "vector_dimensions": os.getenv("MONGODB_VECTOR_DIMENSIONS", "1536"),
    "vector_collection_name": os.getenv("MONGODB_VECTOR_COLLECTION", "rag"),
    "vector_index_name": os.getenv("MONGODB_VECTOR_INDEX", "vector_index"),
}

# For backward compatibility with mongo.py
MONGODB_URL = MONGODB_CONFIG["openai_host"]
DATABASE_NAME = MONGODB_CONFIG["openai_db_name"]

# EMBEDDING CONFIG
FANO_URL = os.environ.get("FANO_URL", "https://portal-demo.fano.ai")
FANO_API_KEY = os.environ.get("FANO_API_KEY", "")


RAG_EMBEDDING_CONFIG: Dict[str, Any] = {
    "maxResponseTokens": int(os.getenv("RAG_EMBEDDING_MAX_RESPONSE_TOKENS", "3000")),
    "chunkSize": int(os.getenv("RAG_EMBEDDING_CHUNK_SIZE", "512")),
    "chunkOverlap": int(os.getenv("RAG_EMBEDDING_CHUNK_OVERLAP", "150")),
    "embeddingModel": os.getenv(
        "RAG_EMBEDDING_MODEL", "sentence-transformers/all-MiniLM-L6-v2"
    ),
    # Bedrock embedding model id (used when LLM_EMBEDDING_PROVIDER=bedrock).
    # Titan v1 is 1536-dim, matching the existing rag.embedding column.
    "bedrockEmbeddingModel": os.getenv(
        "RAG_BEDROCK_EMBEDDING_MODEL", "amazon.titan-embed-text-v1"
    ),
    # Blue-green migration knobs: which rag column the ingest/query paths use.
    # During backfill set writeColumn=embedding_v2 while readColumn stays
    # `embedding`; cut over by flipping readColumn to `embedding_v2`.
    "writeColumn": os.getenv("RAG_EMBEDDING_WRITE_COLUMN", "embedding"),
    "readColumn": os.getenv("RAG_EMBEDDING_READ_COLUMN", "embedding"),
    "searchKValue": int(os.getenv("RAG_SEARCH_K_VALUE", "16")),
}

# Whitelist of valid rag embedding columns. Column names cannot be passed as
# bind parameters, so any column read from config MUST be validated against
# this set before interpolation into SQL.
RAG_EMBEDDING_COLUMNS = {"embedding", "embedding_v2"}

# LLM STUDIO CONFIG
LLM_STUDIO_CONFIG: Dict[str, Any] = {
    "host": os.getenv("LLM_STUDIO_HOST", "http://localhost:8000"),
    "embeddingModel": os.getenv(
        "LLM_STUDIO_EMBEDDING_MODEL", "text-embedding-nomic-embed-text-v1.5"
    ),
    "retrieverModel": os.getenv("LLM_STUDIO_RETRIEVER_MODEL", "lama3.2"),
}
...
OLLAMA_CONFIG: Dict[str, Any] = {
    ...
    "embeddingModel": os.getenv("OLLAMA_EMBEDDING_MODEL", "llama3.2"),
    "retrieverModel": os.getenv("OLLAMA_RETRIEVER_MODEL", "llama3.2"),
    "supported_tool_call_models": os.getenv(
        "OLLAMA_SUPPORTED_TOOL_CALL_MODELS", "llama3.1,mistral"
    ).split(","),
}
...
WORKFLOW_CONFIG: Dict[str, Any] = {
    "host": os.getenv("WORKFLOW_HOST", "http://localhost:8001"),
    "ap_host": os.getenv("AP_WORKFLOW_HOST", "http://localhost:9983"),
    "ap_webhook_url": os.getenv("AP_WORKFLOW_WEBHOOK_URL", "http://localhost:9983"),
}

# BACKEND CONFIG
BACKEND_CONFIG: Dict[str, Any] = {
    "host": os.getenv("BACKEND_HOST", "http://localhost:8002"),
}

DATABOARD_CONFIG : Dict[str, Any] = {
    "host": os.getenv("DATABOARD_HOST", "http://localhost:8081"),
}

# LLM CONFIG
LLM_CONFIG: Dict[str, Any] = {
    "defaultModel": os.getenv("LLM_DEFAULT_MODEL", "gpt-4o"),
    "provider": os.getenv("LLM_PROVIDER", "openai"),
    "default_provider": os.getenv("LLM_DEFAULT_PROVIDER", "openai"),
    "embedding_provider": os.getenv("LLM_EMBEDDING_PROVIDER", ""),
    "retriever_provider": os.getenv("LLM_RETRIEVER_PROVIDER", ""),
    "maxToolCalls": os.getenv("LLM_MAX_TOOL_CALLS", 5),
}

# KIMI CONFIG
KIMI_CONFIG: Dict[str, Any] = {
    "host": os.getenv("KIMI_URL", "http://localhost:9000/v1"),
    "model": os.getenv(
        "KIMI_MODEL", "moonshotai/Kimi-VL-A3B-Thinking-2506"
    ),
}

VLLM_CONFIG: Dict[str, Any] = {
    "host": os.getenv("VLLM_URL", "http://localhost:9000/v1"),
    "model": os.getenv(
        "VLLM_MODEL", "moonshotai/Kimi-VL-A3B-Thinking-2506"
    ),
    "api_key": os.getenv("VLLM_API_KEY", ""),
    "embeddingModel": os.getenv("VLLM_EMBEDDING_MODEL", "llama3.2"),
    "embeddingHost": os.getenv("VLLM_EMBEDDING_URL", ""),
    "retrieverModel": os.getenv("VLLM_RETRIEVER_MODEL", "llama3.2"),
}
...
BEDROCK_CONFIG: Dict[str, Any] = {
    ...
    "model": os.getenv("BEDROCK_MODEL", "qwen.qwen3-32b-v1:0"),
}
