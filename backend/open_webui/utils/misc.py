...
def parse_ollama_modelfile(model_text):
    ...
    for param, param_type in parameters_meta.items():
        param_match = re.search(
            rf"PARAMETER {param} (.+)", model_text, re.IGNORECASE)
        if param_match:
    ...
    message_matches = re.findall(
        r"MESSAGE (\w+) (.+)", model_text, re.IGNORECASE)
...
def generate_ai_assistant_instruction(
    personality_role="",
    core_task="",
    tone_and_style="",
    response_length="",
    ...
    instructions = "Please strictly follow the following settings for the AI Assistant:"
    ...
    if other_requirements and len(other_requirements) > 0:
        instructions += "\n- Other Requirements: \n"
        for idx, item in enumerate(other_requirements):
            requirement = item.get("requirement") if isinstance(
                item, dict) else str(item)
            instructions += f"{idx + 1}. {requirement} \n"

    return instructions


def fix_time_zone(data: dict, utc: str | None) -> dict:
    """Adjusts timezone in the data. (Stub)"""
    print(f"Fixing timezone for data with UTC offset: {utc}")
    # This is where you would implement your timezone adjustment logic
    if utc and "extracted_on" in data:
        data["extracted_on"] = f"{data['extracted_on']}{utc}"
    return data