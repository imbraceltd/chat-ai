...
from open_webui.constants import TASKS, TOOLS
...
async def chat_completion_tools_handler(
    ...
    def get_tools_function_calling_payload(messages, task_model_id, content):
        ...
        prompt = f"History:\n{history}\nQuery: {user_message}"
        # log.info(f"System Prompt: {content}")
        return {
            "model": task_model_id,
            "messages": [
...
async def chat_completion_files_handler(
    ...
    # log.info(f"direct connection to model: {models}")
    
    model_id = body["model"]
    
    if (len(body.get("messages", [])) == 1 and
        body["messages"][0].get("role") == "user" and 
        not body["messages"][0].get("content")):
        # Set default content for empty message
        log.info("using default content for empty message") 
        body["messages"][0]["content"] = "Summarize file contents"

    model = models[model_id]
    log.info(f"chosen model: {model}")
    if model.get("owned_by") == "ollama":
        model["key"] = "ollama"
        model["url"] = request.app.state.config.OLLAMA_BASE_URLS[0]
    else:
        log.info(f"openai info {request.app.state.config.OPENAI_API_BASE_URLS}")
        if model.get("urlIdx") is None:
            model_info = Models.get_model_by_id(model_id)
            if model_info:
                if model_info.base_model_id:
                    model_details = request.app.state.OPENAI_MODELS.get(model_info.base_model_id)
                    idx = model_details["urlIdx"]
                    model["id"] = model_info.base_model_id
        else: 
            idx = model["urlIdx"]
        key = request.app.state.config.OPENAI_API_KEYS[idx]
        url = request.app.state.config.OPENAI_API_BASE_URLS[idx]
        # log.info(f"openai info {key} {url}")
        model["key"] = key
        model["url"] = url  
    ...
    if (len(body.get("messages", [])) == 1 and
        body["messages"][0].get("role") == "user" and 
        body["messages"][0].get("content") == "Summarize file contents"):
        # Set default content for empty message
        log.info("update default message") 
        body["messages"][0]["content"] = "Summarize file contents in a professional business format with bullet points including 'original_filename' and suggest appropriate actions to analyze the file. Please use simple and concise language."

    if (len(body.get("messages", [])) >0):
        body["messages"].append({"role": "assistant", "content": "Please format all tabular data in table format with headers and rows by default. Please use simple and concise language. Ignore this action if the user has specified a different format requirement."})
    return body, {"sources": sources}
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if assistant_id is None and "function_calling" not in metadata:
        metadata["function_calling"] = "native"
    log.info(f"assistant_id: {assistant_id}")
    if TOOLS.VISUALIZE_DATA not in tool_ids and assistant_id is None and files is not None:
        tool_ids.append(TOOLS.VISUALIZE_DATA)
    log.info(f"tool_ids ${tool_ids}")
    ...
    if tool_servers:
        ...
        # If the visualize_data tool should be added forcefully as a fallback
    # log.info(f"tools_dict: {tools_dict}")
    # if TOOLS.VISUALIZE_DATA not in tools_dict:
    #     # Import the tool class
    #     log.info("Importing visualize_data tool")
    #     from open_webui.tools.analyze_tabular_file import Tools as VisualizeTools
        
    #     # Instantiate the tool
    #     visualize_tool = VisualizeTools()
        
    #     # Get the function object
    #     visualize_function = getattr(visualize_tool, "visualize_data_with_echarts")
        
        
    
    if tools_dict:
        log.info(f"function calling: {metadata}")
...
async def process_chat_response(
    ...
    if not any(
        ...
        # log.info(f"Non standard response: {response}")
        return response
                ...
                async def stream_body_handler(response):
                    ...
                    async for line in response.body_iterator:
                        ...
                        try:
                            ...
                            if data:
                                if "sources" in data:
                                    # Emit sources as a special chat completion event
                                    await event_emitter(
                                        {
                                            "type": "chat:completion",
                                            "data": {
                                    ...
                                    Chats.upsert_message_to_chat_by_id_and_message_id(
                                        ...
                                        {
                                            "sources": data["sources"],
                                            "params": {"assistant_id": metadata.get("assistant_id")}
                                        }
                                    )
                                    
                                    continue  # Skip to next data item
